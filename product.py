...
class productClass:
	def __init__(self,root):
		...
		self.root.title("Inventory Management System | Developed by STAR GROUP")
		...
		text_name=Entry(product_frame,textvariable=self.var_name,font=("goudy old style",15),bg='lightyellow').place(x=150,y=160,width=200)
		text_price=Entry(product_frame,textvariable=self.var_price,font=("goudy old style",15),bg='lightyellow').place(x=150,y=210,width=210)
		text_qty=Entry(product_frame,textvariable=self.var_qty,font=("goudy old style",15),bg='lightyellow').place(x=150,y=260,width=200)
			...
			if self.var_pid.get()=="": 
				...
				if row==None:
					...
					if op==True:
						...
						con.commit()
						messagebox.showinfo("Delete","Product Deleted Succcesfully",parent=self.root)
						self.clear()
