...
class employeeClass:
	def __init__(self,root):
		...
		cmb_gender=ttk.Combobox(self.root,textvariable=self.var_gender,values=("Select","Male","Female","Prefer not to say"),state='readonly',justify=CENTER,font=("times new roman",15))
		...
		scrollx=Scrollbar(emp_frame,orient=VERTICAL)
		...
		self.EmployeeTable.heading("adress",text="Address")
			...
			if self.var_emp_id.get()=="": 
				...
				if row!=None:
					...
					cur.execute("Insert into employee(eid,name,email,gender,contact,dob,doj,pass,utype,adress,salary)values(?,?,?,?,?,?,?,?,?,?,?)",(
												...
												self.var_email.get(),													
												self.var_gender.get(),
												self.var_contact.get(),										# INSERT INTO TABLE_NAME [(column1, column2, column3,...columnN)]  
		...
		try:
			if self.var_emp_id.get()=="": 
				...
				if row==None:
					...
					cur.execute("Update employee set name =?,email=?,gender=?,contact=?,dob=?,doj=?,pass=?,utype=?,adress=?,salary=? where eid=?",(
												self.var_name.get(),
												self.var_email.get(),							# UPDATE table_name
			...
			if self.var_emp_id.get()=="": 
				...
				if row==None:
					...
					if op==True:
						...
						con.commit()
						messagebox.showinfo("Delete","Employee Deleted Succcesfully",parent=self.root)
						self.show()
